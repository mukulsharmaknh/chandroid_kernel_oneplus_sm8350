...
static long ksu_sys_fstat(const struct pt_regs *regs)
{
    ...
    if (is_rc) {
        ...
        long size, new_size;
        size_t extra = ksu_rc_len + module_rc_len;
        // Mukul 9RT: copy_*_nofault don't exist on 5.4, use probe_kernel_* instead
        if (!probe_kernel_read(&size, st_size_ptr, sizeof(long))) {
            new_size = size + extra;
            pr_info("adding rc len: %ld -> %ld", size, new_size);
            if (!probe_kernel_write(st_size_ptr, &new_size, sizeof(long))) {
